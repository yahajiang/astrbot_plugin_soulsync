...
### v3.11 指令精简版（当前版本）
- **命令精简**：普通用户子命令 40→18（-55%），/心管 子命令 15+→8（-47%），父命令 10→9
- **迁移至 WebUI**：/回忆 报告/对比/时间线/危机/关系网 · /天象 全部 · /角色 列表/创建/切换/删除 · /人格 设置/重置/锁定 · /知识 删除 · /心声 系统 · /心管 导出/人格锁定/修复统计
- **/心管 统一入口**：`设置 <ID> <字段> <值>`（好感/亲密/态度/关系/角色/图片模式）· `跳跃 <ID> <天数>`（天数=0 重置）· `调试 [ID]`（综合诊断）
- **排行隐私模式**：`rank_privacy_mode` 配置项（self_only/anonymous_top/full），默认仅显示自己排名
- **迁移提示**：已移除命令自动返回引导文案「⏳ 该功能已迁移至 WebUI 控制台」
- **版本号**：metadata 3.10 → 3.11，`_info_header` 同步

### v3.10 Prometheus 沟通感知系统（上一版本）
...
### v2.22 命令总览排版重构与张力/渲染调优（上一版本）
- **命令帮助图片卡片排版重构**：`/心助` 命令总览图片全面重排——分节徽章（父命令+子命令网格、每节示例条）+ 两列子命令网格 + 独立命令段（图片模式/设置）+ 引导框（查询类自然语言直达提示）+ 底部提示条
- **渲染统一修复**：满/空进度条统一按圆角轨道绘制（修复边缘视觉差异）；情感自画像 `╔╝` 框线行样式化为横幅与分隔线（卡面风格统一）
- **张力微变化种子增强**：普通聊天张力积累提速——负面情绪基线由 60 降至 55 起算（系数 0.02→0.03、封顶 3），喜悦偏低基线 40→45 加重种子，新增喜悦 >60 时自然释放（×0.02），日常聊天张力起伏更明显
- **关键词增量上限翻倍**：joy/trust/anticipation 上限 3→6、sadness/anger 上限 4→8，高好感反馈更即时
- **张力显示精度**：卡片张力显示由 `.0f` 改为 `.1f`（微变化可见）
- **版本号**：metadata 2.21 → 2.22，`_info_header` 同步

### v2.21 配置整合与 WebUI 配置板块完善（上一版本）
- **复合评分归一化展示**：新增综合契合度 0~100 归一化（复合评分 -100~215 → 0~100），`/心声 好感/画像` 与档案展示同步显示；内部算法与阶段判定逻辑不变
- **纪念日联动增强**：`/纪念` 记忆联动扩展——私人记忆（promise 内容 + 约定截止日期 promise_due，支持 YYYY-MM-DD）一并纳入日期扫描；知识库自动落库支持 `.` 分隔日期（如 8.8）并按内容区分生日（birthday）/纪念日（anniversary）类型
- **张力微变化驱动**：无关键词命中时的自然互动微变化增加张力种子——压抑的负面情绪（愤怒/悲伤/厌恶/恐惧偏高、喜悦偏低）在普通聊天中持续微积累张力（情绪传染），正面情绪自然释放，日常聊天也能感知张力起伏
- **渲染修复**：8 维情感图标统一为双宽 emoji（期待 ✨ → 🌟），修复画像图片渲染时「期待」行进度条错位
- **命令帮助图片卡片**：`/心助` 命令总览与 `/心管` 帮助支持图片卡片渲染（ADMIN_HELP 多行化，文本展示同步优化）
- **元数据精简**：`metadata.yaml` desc 移除版本历史堆叠，改为精炼特性概述
- **纪念日记忆联动**：`/纪念` 展示知识库中记忆到的生日（自动倒计时，库内已登记生日不重复）与带日期的约定/节日条目
- **命令菜单子命令提示**：14 个顶层命令注册 desc 描述（WebUI 命令菜单显示子命令提示）；README 命令表新增权限列（👤 用户 / 🛡️ 管理员），管理员命令（`/心管`、`/人格 设置/重置/锁定`、`/纪念 节日添加/节日删除`）单列标注；修正 `/环境` → `/天象`
- **RDE 阶段叙事频控注入**：`rde_stage_inject_every_n`（默认 3 轮）按间隔注入阶段叙事（含称谓/禁忌等行为指导），阶段跃迁轮强制注入，危机叙事与关系感知不受间隔限制；设为 0 恢复每轮注入
- **阶段跃迁静态层重注**：阶段跃迁时强制重新注入静态层（人设/知识），确保称谓/人格随阶段即时演进
- **WebUI 配置板块补全**：配置面板 13 组 92 键 → **16 组 141 键全量覆盖**（新增 RDE 扩展 13、个性化训练 14、TPD 22，消灭全部缺失键）；`.cs` 配置组玻璃卡片化（渐变背景/圆角/阴影/标题徽章/行 hover），风格统一；控制台按钮文案同步「141 项参数 · 16 个模块组」
- **配置模块分类**：`_conf_schema.json` 新增 `_section_rde` 分组标记，功能开关键集中头部；`_info_header` 版本 v2.18 → v2.21
- **插件元数据**：插件名称定为「心旅知音 (SoulSync) - 融合版情感智能插件」，version 2.21
- **命令换词（避免与其他插件指令冲突）**：`/状态`→`/心声`、`/回顾`→`/回忆`、`/环境`→`/天象`、`/命令`→`/心助`、`/admin`→`/心管`（子命令全部不变；纪念/角色/人格/知识/风格/记忆/排行/图片模式/设置 保留）；同步更新路由表、注册、帮助、测试与文档
- **测试**：新增 `tests/test_webui.py`（面板键与 schema 全量互覆盖 + RDE 归组 + 版本校验回归），全量 26 个测试文件通过
