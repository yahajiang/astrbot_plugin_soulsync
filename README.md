...
- [功能总览](#overview) · [安装](#install) · [命令](#commands) · [更新流程](#flow) · [核心机制](#core) · [个性化训练](#trainer) · [RDE 关系演进](#rde) · [WebUI](#webui) · [配置](#config) · [文件结构](#files) · [版本记录](#changelog) · [致谢](#credits)
...
| WebUI 控制台 | 仪表盘 / 自画像 / 141 项配置 · 16 模块组 / 排行榜 / 管理员工具 |
| 个性化训练 | 人格微调（20 参数+护栏）/ 知识库 / 语言风格（三阶段）/ 私人记忆 |
| RDE 关系深度演进 | 十二阶段叙事注入 / 关系危机（7 类型 14 事件）/ 多角色关系网 |
...
**`/心声`** — `好感`（核心数值）· `阶段`（阶段详情）· `画像`（自画像）· `系统`（缓存统计）

**`/回忆`** — `趋势 [天数]`（14 天趋势）· `月报`（月度报告）· `报告/独白 [天数]`（角色回顾）· `对比 [天数]`（六维雷达）· `时间线/危机/关系网 [ID]`（RDE，需开启 `enable_rde`，管理员可查他人）

### 纪念日与角色

**`/纪念`** — `查看`（列表+倒计时）· `添加 <MM-DD> <名称>` · `删除` · `生日 <MM-DD>` · `节日`（节日列表）

🛡️ `节日添加` / `节日删除` — 仅管理员

**`/角色`** — `列表/查看` · `创建 <名字> [emoji] [性格]` · `切换 <名字|默认>` · `删除` · `称谓`（专属称谓）· `关系 [角色]`（关系角色列表+解锁进度）· `解锁 <角色>` · `关系切换 <角色>`（**切换即锁定，不可逆**）

### 个性化训练（需开启 `enable_personalization`）

**`/人格`** — `查看`（面板，全员只读）| 🛡️ `设置` / `重置` / `锁定`（仅管理员，操作后 2h 自动化暂停）

**`/知识`** — `查看` · `添加 [分类] <内容>`（profile/interests/people/promises/experiences/values）· `删除 <ID>`

**`/风格`** — `状态` · `保存/恢复 [名称]` · `锁定`（停止学习）

**`/记忆`** — `查看` · `添加 <类型> <内容>`（text/image/promise/emotional）· `删除/星标 <ID>` · `重要/忘记 [序号]`

### 环境感知与排行

**`/天象`** — `天气`（环境感知）· `倒计时`（临近事件）· `跳跃 [N天后见]` · `回溯`（关键时刻叙事）

**`/排行`** — `好感 [n]` / `负好感 [n]`（TOP n，默认 10）
...
|------|------|
| `/心管 图片模式` | 全局开启/关闭图片输出 |
| `/心管 好感/亲密/态度/关系 <ID> ...` | 强制设置好感、态度、关系描述 |
| `/心管 关系角色 <ID> <角色>` | 强制调整关系角色（绕过锁定） |
| `/心管 重置好感 <ID>` | 重置默认值并清空长期记忆和行为档案 |
| `/心管 查看好感 <ID>` | 查看完整档案（8 维情感/记忆/行为） |
| `/心管 隐私 <0-2>` | 0=完全保密 1=基础 2=详细 |
| `/心管 重置` / `备份` | 清空所有数据 / 创建快照 |
| `/心管 修复统计` | 依据好感自动修正正负互动次数 |
| `/心管 强制跳跃 <ID> <天数>` / `重置跳跃 [ID]` | 强制时间跳跃 / 重置状态 |
| `/心管 天气调试` | 天气 API/缓存调试信息 |
| `/心管 导出` | 导出个性化数据为 JSON |
| `/心管 人格锁定` | 锁定/解锁人格参数 |
| `/心管 调试事件` / `调试记忆` | 输出事件结构 / 记忆与档案详情 |
...
| 管理员调整 | `/心管 关系角色` 或 WebUI 强制调整 |

- 解锁需好感、亲密、互动次数同时满足
- 负好感自动定级（世仇→仇人→对手→厌恶→反感→冷漠）
- 自定义态度/关系合并进当前角色人设
...
开启 `enable_personalization` 后生效（默认关闭）：

| 模块 | 说明 |
|------|------|
| 人格微调 | 20 参数，隐式训练 + 稳定度半衰期防抖，锁定后不可改 |
| 知识库 | 6 类（基本信息/兴趣/人物/约定/经历/价值观），自动提取 + 手动增删 |
| 语言风格 | 三阶段训练（采集→模仿→融合），快照保存/恢复 |
| 私人记忆 | 4 类型（文字/图片/约定/情感）+ 星标，显著性阈值自动提取 |

**四模块联动**：注入优先级 人格 > 知识 > 记忆 > 风格（450 token 预算）；记忆 importance≥8 沉淀为知识；promises 自动联动纪念日；人格参数加权检索；joy/trust 基线注入情感引擎。

**人格护栏（v2.20）**：连续 50 轮无波动+稳定度≥70% → 自动锁定；24h 内≥3 次剧变 → 回滚；背叛/冷落≥3 天 → 自动解锁；`/人格 设置` 后 2h 临时锁定。

**存储**：`data/personalization/{user_id}/` 每用户独立 JSON（原子写入 .tmp+.bak，目录上限 5MB）。
...
开启 `enable_rde` 后生效（默认关闭）：

| 子系统 | 说明 |
|--------|------|
| 十二阶段叙事 | s1~s12（初识→共生）+ n1~n4（冷淡→敌对），每阶段独立称谓/口吻/叙事注入 |
| 关系危机 | 7 类型 14 事件，概率+阶段/好感/冷落/节日修正因子触发，仅危机可致阶段倒退 |
| 多角色关系网 | 39 角色稀疏矩阵，跨角色好感传导（延迟一轮），5 类社交事件 |

每轮流程：危机检测 → 跨角色传导 → 阶段叙事 → 三段上下文注入。性能 **<0.01ms/轮**。

**叙事频控（v2.21）**：阶段叙事按 `rde_stage_inject_every_n`（默认 3 轮）间隔注入，跃迁轮强制；危机与关系感知不受限。

**子开关**：`enable_crisis_system` / `enable_network`。自定义危机事件见 `rde/crisis/crisis_definitions.py`，自定义关系写在角色卡 `relations` 字段。
