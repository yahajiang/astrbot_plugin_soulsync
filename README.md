...
| 权限 | 子命令 | 说明 |
|------|------|------|
| 👤 | `好感 [n]` / `负好感 [n]` | TOP n 好感 / BOTTOM n 负好感排行榜（默认 10，最多 20） |
...
> `enable_rde` 开启后 `/回忆 时间线/危机/关系网` 才可用；`/心助` 总览按权限过滤：普通用户不显示节日添加/删除、人格 设置/重置/锁定 与 `/心管`。
...
| 版本 | 摘要 |
|------|------|
| **v2.23**（当前） | `/心助` 命令总览按权限过滤（普通用户隐藏节日添加/删除、人格设置/重置/锁定、/心管；管理员全显）· 帮助说明按 README 措辞全面扩充（分类枚举/权限标注/默认值） |
| **v2.22** | `/心助` 命令总览图片排版重构（分节徽章+两列子命令网格+引导框）· 进度条/框线渲染统一 · 张力种子增强（基线55×0.03 封顶3+喜悦释放）· 关键词增量上限翻倍 · 张力显示 1 位小数 |
| **v2.21** | RDE 阶段叙事频控注入（间隔 3 轮/跃迁强制，危机感知不受限）· 阶段跃迁静态层重注 · WebUI 配置板块 16 模块组 141 键全量可视化（.cs 玻璃卡片化）· schema `_section_rde` 分组 · 插件名称定为「心旅知音 (SoulSync) - 融合版情感智能插件」 |
