...
| Prometheus 沟通感知（v3.10） | 25窗滑动权重漂移（实时感知沟通习惯）· 离线灵魂素描（10条棱镜+沟通建议）· 单闭环基准线修正 |
...
> 📡 更多功能请前往 **WebUI 控制台**操作（角色管理、人格微调、深度分析、环境感知等）。
...
> 排行隐私模式：`rank_privacy_mode`（self_only 默认仅显示自己排名 / anonymous_top 匿名化 / full 全部显示）。
...
| 权限 | 命令 | 说明 |
|------|------|------|
| 🛡️ | `/心管 设置 <ID> <字段> <值>` | 统一设置（favor/intimacy/attitude/relationship/role/image_mode） |
| 🛡️ | `/心管 查看 <ID>` | 完整档案（8 维+记忆+行为模式） |
| 🛡️ | `/心管 重置 <ID>` | 清空该用户全部数据 |
| 🛡️ | `/心管 跳跃 <ID> <天数>` | 强制时间跳跃（天数=0 重置冻结） |
| 🛡️ | `/心管 压缩 <ID>` | 手动触发记忆压缩（v3.00） |
| 🛡️ | `/心管 隐私 <ID> <0-2>` | 0=完全保密 1=基础 2=详细 |
| 🛡️ | `/心管 备份` | 创建全局数据快照 |
| 🛡️ | `/心管 调试 [ID]` | 综合诊断（环境/天气/事件/记忆/冷落） |
...
├── prometheus/            # v3.10 Prometheus 沟通感知
│   ├── sliding_window.py      # 25窗滑动权重漂移
│   ├── psych_store.py         # 心理画像存储
│   └── soul_sketch.py         # 离线灵魂素描（棱镜生成）
...
| 版本 | 摘要 |
|------|------|
| **v3.11**（当前） | 指令精简：普通用户 40→18 子命令 · 管理员 15+→8 · 功能迁移 WebUI · 排行隐私模式 |
| **v3.10** | Prometheus：25窗滑动权重漂移 · 离线灵魂素描（棱镜+沟通建议）· 单闭环基准线修正 |
| **v3.00** | Project Hermes：SQLite 分片存储 · 转生系统 · 记忆压缩 · 降级熔断 · 排行榜缓存 |
