# 心旅知音 (SoulSync) v2.20 - 融合版情感智能插件（自用）
...
### v2.20（当前版本）
- **性能优化（观感零损失）**：
  - **滚动滑块改为合成器驱动**：CSS scroll-driven animation（`animation-timeline:scroll()`），滑块位置由浏览器合成线程直接跟随滚动，彻底解决主线程卡顿导致的滑块滞后/跳动（滚动时每帧零 JS），内容/尺寸变化时 JS 仅低频更新 `--mm-y` 变量；不支持时自动回退旧 rAF 方案
  - **移除按钮层 backdrop-filter**：`.btn` 白 65% 底在浅色背景上 blur 不可见，删除后常驻重采样层数大减（保留卡片/统计卡玻璃核心 + 弹窗/遮罩惰性模糊 + transform-only 光斑漂移）
- **修复**：滚动滑块不同步（滑块滞后、跳动）

### v2.19
- **WebUI 全量重构：浅色日景玻璃拟态**（Glassmorphism 风格体系）：
  - **Token 体系**：统一 CSS 变量（文字/金色强调/玻璃透明度/方向性阴影/spring 缓动），全站 15 处动效统一 `duration-500 cubic-bezier(.16,1,.3,1)`
  - **真实玻璃光学**：卡片/统计卡/弹窗高模糊（blur 40-60px）+ `saturate(180%)` + 方向性阴影（外投影 + inset 顶边白高光 + 底边暗缘），卡片顶部渐变高光模拟受光
  - **香槟金唯一强调色**：深香槟金 `#A67C2F`（浅色背景 WCAG AA 适配），仅用于主要动作按钮/分区标题/滚动条拖拽态；主按钮由粉紫渐变改为纯色金
  - **移除紫粉渐变**：粉色/紫色光斑改为蓝灰+香槟金克制光源；渐变文字标题移除（规范禁止项）
  - **噪点质感**：背景 2.5% SVG feTurbulence 噪点消除塑料感
  - **动效规范化**：移除全部 bounce/elastic 弹跳缓动，hover lift/active scale(.97)/弹窗 spring 弹入
  - 滚动条滑块（Minimal Modern）金色适配；`prefers-reduced-motion`/`@supports` 降级保留
