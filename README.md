...
> 💡 **95% 场景无需输入命令**：查询类自然语言（如"我们之间现在算什么关系？""你还记得我们第一次见面吗"）由意图识别自动输出卡片，无需输入 `/命令`。`/命令` 查看总览。

| 父命令 | 子命令 | 说明 |
|------|------|------|
| `/状态` | `好感` | 核心情感数值（好感/亲密度/阶段/行为势头） |
| | `阶段` | 当前关系阶段详情 |
| | `画像` | 个人情感自画像（8 维情感 + 行为模式 + 里程碑） |
| | `系统` | 插件数据规模（缓存统计） |
| `/回顾` | `趋势 [天数]` | 最近 N 天（默认 14）情感数据趋势 |
| | `月报 [上月]` | 关系月报（净好感/情绪主色调/里程碑） |
| | `报告 [天数]` / `独白` | 角色第一人称口吻回顾（默认 14 天） |
| | `对比 [天数]` | 前后两段 N 天关系六维雷达对比（默认 7 天） |
| | `时间线 [ID]` | RDE 阶段时间线叙事；管理员加 ID 查看他人 |
| | `危机 [ID]` | RDE 危机历史与冷却；管理员加 ID 查看他人 |
| | `关系网 [ID]` | 多角色关系网；管理员加 ID 查看他人 |
| `/纪念` | `查看` | 纪念日列表与倒计时 |
| | `添加 <MM-DD> <名称>` | 添加自定义纪念日 |
| | `删除 <名称>` | 删除自定义纪念日 |
| | `生日 <MM-DD>` | 设置生日 |
| | `节日` | 全部节日列表与倒计时 |
| | `节日添加 <名称> <MM-DD>` | 管理员添加节日 |
| | `节日删除 <名称>` | 管理员删除节日 |
...
| | `删除 <ID>` | 删除记忆 |
| | `星标 <ID>` | 切换记忆星标（⭐ 检索优先） |
| | `重要 <序号>` | 把长期记忆标记为重要（1=最近） |
| | `忘记 <序号>` | 忘掉长期记忆（1=最近） |
| `/环境` | `天气` | 当前环境感知（天气/季节/节气/月相/心情倾向） |
| | `倒计时` | 即将到来的倒计时事件（类型/距离/得分） |
| | `跳跃` | 时间跳跃状态；`/环境 跳跃 三天后见` 触发跳跃 |
| | `回溯` | 回溯关键时刻的时间线叙事（最多 5 条） |
| `/排行` | `好感 [n]` | TOP n 好感排行榜（默认 10，最多 20） |
| | `负好感 [n]` | BOTTOM n 负好感排行榜 |
...
| 命令 | 说明 |
|------|------|
| `/admin 图片模式` | 全局开启/关闭图片输出（所有信息命令强制图片） |
| `/admin 好感 <ID> <值>` | 设置好感度（-100 ~ 200，亲密度随之派生） |
| `/admin 亲密 <ID> <值>` | 亲密度已按好感度派生，此命令提示改用设置好感 |
| `/admin 态度 <ID> <文本>` | 自定义态度描述（合并进当前关系角色人设生效） |
| `/admin 关系 <ID> <文本>` | 自定义关系描述（合并进当前关系角色人设生效） |
| `/admin 关系角色 <ID> <角色>` | 强制调整关系角色（绕过锁定与解锁条件，解除锁定） |
| `/admin 重置好感 <ID>` | 重置为默认值并清空长期记忆和行为档案 |
| `/admin 查看好感 <ID>` | 查看完整档案（含 8 维情感、长期记忆、行为模式） |
| `/admin 隐私 <0-2>` | 0=完全保密 1=基础 2=详细 |
| `/admin 重置` | 清空所有数据（含行为档案） |
| `/admin 备份` | 创建快照（含长期记忆和行为档案） |
| `/admin 修复统计` | 依据好感/亲密度自动修正正负互动次数 |
| `/admin 强制跳跃 <ID> <天数>` | 强制指定用户时间跳跃（冻结冷落惩罚） |
| `/admin 重置跳跃 [ID]` | 重置用户跳跃状态（偏移归零、解冻） |
| `/admin 天气调试` | 查看天气获取调试信息（API/缓存/来源） |
| `/admin 导出` | 导出个性化数据为 JSON（persona/knowledge/style/memory） |
| `/admin 人格锁定` | 锁定/解锁指定用户人格参数 |
| `/admin 调试事件` / `调试记忆` | 输出事件结构 / 近期对话缓存、长期记忆、行为档案详情（排障） |
...
`/状态 画像` 或 WebUI 点击用户卡片，展示完整自画像：核心数值（好感/亲密度/复合评分）、关系阶段与角色、自定义态度/关系描述、8 维情感、互动统计、行为模式（势头/连续/里程碑）、关系建议。
...
内置农历换算（1900-2100）自动识别**认识里程碑**（7/30/50/100/200/365/500/1000/1500/2000/3000 天）、认识周年、生日与 27+ 传统节日；节日当天自动好感/亲密奖励（每日去重）+ LLM 氛围注入；`/纪念` `/纪念 节日` 查看倒计时，WebUI 可视化编辑。
...
信息命令（`/状态` `/状态 画像` `/排行` `/排行 负好感` `/纪念` `/回顾 趋势` `/状态 阶段` `/角色 关系` `/状态 系统` `/admin 查看好感` `/admin 调试记忆`）均可渲染为 Pillow 图片卡片（含双线趋势图）。
...
| 版本 | 摘要 |
|------|------|
| **v2.20**（当前） | 轻量化重构：70 命令收敛为 10 父命令+`/admin`（`command_router.py` 路由表） · 事件总线 EventBus 解耦（`favor.changed`/`stage.advanced` 埋点） · 人格护栏（50 轮稳定自动锁定/24h 3 次剧变回滚/背叛·冷落72h+自动解锁/管理员 2h 临时锁定） · 自然语言意图识别静默命令（`intent_router.py`：状态/记忆/环境/排行/纪念日查询免命令出卡片） · 前后置钩子机制（`hook_bus.py`，意图识别与 prompt 泄漏清理挂载） · 127 组测试 |
| **v2.19** | 时间感知深化（TPD）：三级降级天气获取+8维心情映射 · 倒计时六阶段叙事（T-7~T+7） · 时间跳跃叙事（告别/回归/被动离开6级分级） · 冷落惩罚冻结 · 22项TPD配置 · 6命令+图片模式 · WebUI时间感知面板 · 74组测试 · 性能1.14ms/轮 |
| **v2.18** | 关系深度演进（RDE）：十二阶段叙事注入 · 关系危机系统（7类型14事件） · 多角色关系网（跨角色好感传导+5类社交事件） · 3命令 · WebUI RDE面板 · 14项配置 |
| **v2.17** | 个性化训练模块：人格微调（20 参数隐式训练+稳定化+锁定）、知识库（6 类知识+情景捕捉）、语言风格（三阶段训练+快照）、私人记忆（4 类型+星标+审计）· 四模块联动（token 预算裁剪/记忆↔知识↔纪念日/长期记忆沉淀/人格加权检索/joy·trust 基线注入辅助 LLM）· 15 个新命令 · WebUI 个性化训练面板（4 标签页+5 组 trainer API） |
